...
#if defined(__aarch64__) && defined(CIA3DS_HW_AES)
#include "aes_armv8.h"
#endif
...
exit:
#if defined(__aarch64__) && defined(CIA3DS_HW_AES)
    ctx->hw_dec_rk_valid = 0;
    if( ret == 0 && cia3ds_aes_arm_available() ) {
        cia3ds_aes_armv8_build_dec_schedule( ctx->hw_dec_rk,
                                             (const unsigned char *)cty.rk,
                                             cty.nr );
        ctx->hw_dec_rk_valid = 1;
    }
#endif
...
#if defined(__aarch64__) && defined(CIA3DS_HW_AES)
    if( cia3ds_aes_arm_available() ) {
        if( mode == MBEDTLS_AES_ENCRYPT ) {
            cia3ds_aes_arm_encrypt( ctx->nr,
                                    (const unsigned char *)ctx->rk,
                                    input, output );
            return( 0 );
        } else if( ctx->hw_dec_rk_valid ) {
            cia3ds_aes_arm_decrypt( ctx->nr, ctx->hw_dec_rk,
                                    input, output );
            return( 0 );
        }
    }
