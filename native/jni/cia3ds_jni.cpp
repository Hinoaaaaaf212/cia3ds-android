...
bool copy_range_to_fd(int src_fd, uint64_t start, uint64_t len, int out_fd) {
    constexpr size_t kBuf = 256 * 1024;
    std::unique_ptr<uint8_t[]> buf(new uint8_t[kBuf]);
    uint64_t copied = 0;
    while (copied < len) {
        size_t want = static_cast<size_t>(
            std::min<uint64_t>(kBuf, len - copied));
        ssize_t n = pread(src_fd, buf.get(), want,
                          static_cast<off_t>(start + copied));
        if (n <= 0 || !write_all(out_fd, buf.get(), static_cast<size_t>(n))) {
            return false;
        }
...
bool copy_file_to_fd_exact(const std::string &path, uint64_t expected_size,
                           int out_fd) {
    ...
    bool ok = fstat(fd, &st) == 0
           ...
           && copy_range_to_fd(fd, 0, expected_size, out_fd);
    close(fd);
    return ok;
}

// Rebuilding an NCSD via makerom first creates a CIA and then loads that whole
// multi-gigabyte file into memory for CIA-to-CCI conversion. That is not viable
// on Android. The decrypted partitions have exactly the same sizes as the
// originals, so preserve the source NCSD byte-for-byte and stream each updated
// partition back into its original slot instead.
bool stream_rebuilt_ncsd_to_fd(const std::string &input_path,
                               const std::vector<Partition> &partitions,
                               int out_fd) {
    int src_fd = open(input_path.c_str(), O_RDONLY);
    if (src_fd < 0) return false;

    struct stat src_st{};
    uint8_t hdr[0x100];
    bool ok = fstat(src_fd, &src_st) == 0
           && src_st.st_size >= 0x200
           && pread_full_at(src_fd, hdr, sizeof(hdr), 0x100)
           && memcmp(hdr, "NCSD", 4) == 0;
    if (!ok) {
        close(src_fd);
        return false;
    ...
    uint32_t block_size = static_cast<uint32_t>(0x200u) << hdr[0x8E];
    for (const auto &partition : partitions) {
        if (partition.slot < 0 || partition.slot >= 8) {
            close(src_fd);
            return false;
        }
        const uint8_t *entry = hdr + 0x20 + partition.slot * 8;
        uint64_t offset = static_cast<uint64_t>(rd_u32_le_at(entry))
                        * block_size;
        uint64_t size = static_cast<uint64_t>(rd_u32_le_at(entry + 4))
                      * block_size;
        if (size == 0 || offset > static_cast<uint64_t>(src_st.st_size)
            || size > static_cast<uint64_t>(src_st.st_size) - offset) {
            close(src_fd);
            return false;
        }
        regions.push_back({offset, size, &partition});
    }
    ...
    if (lseek(out_fd, 0, SEEK_SET) == static_cast<off_t>(-1)) {
        close(src_fd);
        return false;
    }
    // File-picker destinations are normally already truncated. This also
    // handles replacing an existing larger file when the provider supports it.
    (void)ftruncate(out_fd, 0);

    uint64_t cursor = 0;
    for (const auto &region : regions) {
        if (region.offset < cursor
            || !copy_range_to_fd(src_fd, cursor, region.offset - cursor, out_fd)
            || !copy_file_to_fd_exact(region.partition->path, region.size,
                                      out_fd)) {
            ok = false;
            break;
        }
    ...
    uint64_t input_size = static_cast<uint64_t>(src_st.st_size);
    if (ok) ok = copy_range_to_fd(src_fd, cursor, input_size - cursor, out_fd);
    close(src_fd);
    return ok;
}
...
Java_io_github_cia3ds_jni_Cia3ds_nativeDecryptCia(
    ...
    if (info.is_3ds && wantCci) {
        if (bail_if_cancelled()) return 13;
        progress.post(80, "Rebuilding 3DS");
        sink.emit("rebuild: streaming decrypted partitions into the original NCSD layout");
        if (!stream_rebuilt_ncsd_to_fd(input_path_for_tools, partitions, outFd)) {
            sink.emit("ERR: could not write the rebuilt 3DS to the chosen output.");
            sink.emit("Most common reasons:");
            sink.emit("  - The destination ran out of free storage.");
            sink.emit("  - The destination folder is no longer writable.");
            sink.emit("  - The source file changed while decryption was running.");
            sink.emit("Free up space, re-pick the input, and try again.");
            return 8;
        }
        struct stat source_st{};
        if (stat(input_path_for_tools.c_str(), &source_st) == 0) {
            sink.emitf("rebuilt NCSD: %lld bytes",
                       static_cast<long long>(source_st.st_size));
        }
        sink.emit("META: format_actual=ncsd");
